	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int i = 0; i < count; i++) {
			...
			while (leftQueue.size() >= 2) {
				...
				if (leftQueue.contains(queueHead + 500) && rightQueue.contains(queueHead + 1000)
						&& rightQueue.contains(queueHead + 1500)) {
					leftNumber++;
					leftQueue.remove(queueHead + 500);
					rightQueue.remove(queueHead + 1000);
					rightQueue.remove(queueHead + 1500);
				} else if (leftQueue.contains(queueHead + 500) && rightQueue.contains(queueHead - 500)
						&& rightQueue.contains(queueHead - 1000)) {
					leftQueue.remove(queueHead + 500);
